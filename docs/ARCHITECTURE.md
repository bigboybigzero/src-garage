...
## ขั้นตอนอ่าน
- **ค้นทะเบียน (4 ชั้น):** ตรงตัว → ทะเบียนที่มีคำค้นอยู่ข้างใน (หรือกลับกัน) → เทียบเฉพาะตัวเลข → ระยะแก้คำ (edit distance) ≤ 2 เอา 6 อันดับแรก · เจอคันเดียวเปิดประวัติเลย เจอหลายคันขึ้นรายการให้เลือก พร้อมบอกว่าจับคู่ด้วยวิธีไหน
- normalize คำค้น → หา vehicle ที่ `plateKey` ตรง → customer จาก `customerId` → jobs จาก `vehicleId` เรียงใหม่→เก่า → รวม `items` เป็น total แล้วคิด balance · ไม่พบ = ข้อความบอกว่าไม่พบ ไม่ใช่หน้าเปล่า
